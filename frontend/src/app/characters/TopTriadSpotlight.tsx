...
export default function TopTriadSpotlight({ topRankList }:TopTriadSpotlightProps) {
  return (
    <div className='flex justify-center items-end mmlg:hidden'>
      <Link href={`./characters/${topRankList?.[1]?.name === undefined ? '' : topRankList?.[1]?.name}`} className='group w-[260px] h-[200px] rounded-[10px] bg-[#15181d] overflow-hidden'>
        <div className='relative w-full h-full opacity-100 translate-x-0'>
          <div className='absolute w-[450px] -left-[90px] group-hover:scale-105 transition-transform duration-700 ease-out -top-[60px]'>
            {topRankList?.[1]?.name && <Image src={topRankList?.[1]?.imgAddress || ''} alt={topRankList?.[1]?.name || ''} width={450} height={521} priority={false} />}
          </div>
      ...
      <Link href={`./characters/${topRankList?.[0]?.name === undefined ? '' : topRankList?.[0]?.name}`} className='group w-[560px] h-[220px] rounded-[10px] bg-[#15181d] overflow-hidden ml-5'>
        <div className='relative w-full h-full opacity-100 translate-x-0'>
          <div className='absolute w-[560px] left-0 group-hover:scale-105 transition-transform duration-700 ease-out -top-[84px]'>
            {topRankList?.[0]?.name && <Image src={topRankList?.[0]?.imgAddress || ''} alt={topRankList?.[0]?.name || ''} width={560} height={648} priority={false} />}
          </div>
      ...
      <Link href={`./characters/${topRankList?.[2]?.name === undefined ? '' : topRankList?.[2]?.name}`} className='group w-[260px] h-[200px] rounded-[10px] bg-[#15181d] overflow-hidden ml-5'>
        <div className='relative w-full h-full opacity-100 translate-x-0'>
          <div className='absolute w-[450px] -left-[90px] group-hover:scale-105 transition-transform duration-700 ease-out -top-[60px]'>
            {topRankList?.[2]?.name && <Image src={topRankList?.[2]?.imgAddress || ''} alt={topRankList?.[2]?.name || ''} width={450} height={521} priority={false} />}
          </div>
          <div className='absolute top-5 left-5'>
